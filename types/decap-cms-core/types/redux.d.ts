...
export type CmsBackendType = 'azure' | 'git-gateway' | 'github' | 'gitlab' | 'gitea' | 'bitbucket' | 'test-repo' | 'proxy';
...
export type CmsCollectionFormatType = 'yml' | 'yaml' | 'toml' | 'json' | 'frontmatter' | 'yaml-frontmatter' | 'toml-frontmatter' | 'json-frontmatter';
...
export type Config = StaticallyTypedRecord<{
    ...
    collections: List<StaticallyTypedRecord<{
        name: string;
    }>>;
}>;
type PagesObject = {
...
export type FilterMap = StaticallyTypedRecord<ViewFilter & {
    active: boolean;
}>;
export type GroupMap = StaticallyTypedRecord<ViewGroup & {
    active: boolean;
}>;
export type Filter = Map<string, Map<string, FilterMap>>;
export type Group = Map<string, Map<string, GroupMap>>;
...
export type Collections = StaticallyTypedRecord<{
    [path: string]: Collection & CollectionObject;
}>;
...
export interface EntriesAction extends Action<string> {
    payload: EntryRequestPayload | EntrySuccessPayload | EntryFailurePayload | EntriesSuccessPayload | EntriesRequestPayload | EntryDeletePayload;
    meta: {
        collection: string;
    };
}
