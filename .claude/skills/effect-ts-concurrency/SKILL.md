...
Effect-TS provides lightweight fibers for high-performance concurrency. The core principle is
**explicit control**: always bound parallelism to prevent resource exhaustion.
...
Unbounded parallelism is the most common source of "Too many open files" or "Connection timeout"
errors.
...
- **Leaking Fibers:** Forking fibers without joining or interrupting them (use `Effect.scoped` for
  safety).
