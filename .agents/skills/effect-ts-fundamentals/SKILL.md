...
Effect-TS provides a unified toolkit for building robust applications by treating side effects as
values. It enables type-safe error handling, dependency injection, and resource management.
...
Effect replaces manual dependency passing and nested `try/catch` with a declarative, generator-based
flow.
...
- **Forgetting `yield*`**: Calling an effect without `yield*` inside `Effect.gen` returns the effect
  itself, not its value.
- **Manual try/catch**: Use `Effect.try` or `Effect.catchTag` instead of standard `try/catch` to
  keep errors in the type channel.
