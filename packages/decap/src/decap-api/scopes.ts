/**
 * Decap-specific authorization policy over the shared scope mechanism.
 *
 ...
 * only the decap-api-specific pieces: the `domain`+`operation` -> scope mapping
 * and `createScopePolicy()`, an opt-in `authorize()`.
 *
 * `authorize(ctx)` stays the single, consumer-owned access decision (see
 * {@link DecapOptions.authorize}); this is a convenience so the common case is
...
// Re-export the shared scope mechanism so decap-api consumers reach it from one
