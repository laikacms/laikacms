...
export const IconControl: React.FC<IconControlProps> = props => {
  ...
  const [allIcons, setAllIcons] = useState({} as Record<string, React.ForwardRefExoticComponent<IconProps & React.RefAttributes<SVGSVGElement>>>)

  useEffect(() => {
    import("@radix-ui/react-icons").then((module) => {
      const icons = Object.fromEntries(Object.entries(module).filter(([key]) => key.endsWith("Icon")))
      setAllIcons(icons as Record<string, React.ForwardRefExoticComponent<IconProps & React.RefAttributes<SVGSVGElement>>>);
    });
  }, [])
