...
export class GenericScanner implements DangerousContentScanner {
  ...
  readonly supportedTypes: readonly DetectedFileType[] = [
    'unknown', 'mp4', 'mov', 'avi', 'tiff', 'heic', 'heif', 'pdf',
  ];
  ...
  private scanXmpContent(data: Uint8Array, startOffset: number): ScanResult {
    ...
    let endOffset = this.findSequence(data, [0x3C, 0x3F, 0x78, 0x70, 0x61, 0x63, 0x6B, 0x65, 0x74, 0x20, 0x65, 0x6E, 0x64], startOffset); // <?xpacket end
    
    if (endOffset === -1) {
      // Try to find </x:xmpmeta>
      endOffset = this.findSequence(data, [0x3C, 0x2F, 0x78, 0x3A, 0x78, 0x6D, 0x70, 0x6D, 0x65, 0x74, 0x61], startOffset);
    }
