...
import { readUint32LE, writeUint32LE, concatBytes, sliceBytes } from '../utils/binary.js';
...
export class WebpSanitizer implements FileSanitizer {
  ...
  async sanitize(data: Uint8Array, _options: SanitizeOptions): Promise<SanitizeResult> {
    ...
    const hasImageData = keptChunks.some(c => 
      c.fourCC === 'VP8 ' || c.fourCC === 'VP8L' || c.fourCC === 'ANMF'
    );
