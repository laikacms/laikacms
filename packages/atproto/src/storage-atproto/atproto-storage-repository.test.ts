...
const uriFor = (collection: string, rkey: string): string =>
      ...
      if (w.$type === 'com.atproto.repo.applyWrites#create') {
        ...
        mutations.push(() => {
          store.set(makeKey(w.collection, w.rkey), {
            uri: uriFor(w.collection, w.rkey), cid, collection: w.collection, rkey: w.rkey, value: w.value,
          });
        });
        results.push({
          $type: 'com.atproto.repo.applyWrites#createResult',
          uri: uriFor(w.collection, w.rkey), cid, validationStatus: 'valid',
        });
        ...
        mutations.push(() => { store.delete(makeKey(w.collection, w.rkey)); });
        results.push({
