...
export class AtprotoStorageRepository extends StorageRepository {
   ...
   */
  private async resolveFile(key: string): Promise<{ record: AtprotoRecord<StoredAtomValue>; extension: string } | null> {
      ...
      Effect.gen({ self: this }, function*() {
        ...
        if (update.content) {
          ...
          yield* liftResult(this.dataSource.putRecord(
            this.fileCollection, rkey, newValue,
          ...
          for (let i = 0; i < results.length; i += 1) {
            ...
            if (r && r.validationStatus && r.validationStatus !== 'valid') {
              yield* emit.recoverableError(
                new InternalError(`AT Protocol applyWrites validation failed for ${found[i]!.key}: ${r.validationStatus}`),
              );
            }
          }
        }
  ...
  getCapabilities(): LaikaTask.LaikaTask<Capabilities> {
    return LaikaTask.succeed<Capabilities>({
      ...
      fileExtensions: {
        supported: true,
        description: 'Each object is one AT Protocol record in the file collection; the extension is encoded into the rkey tail and stored in the record value.',
