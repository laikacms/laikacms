...
export class SolidStorageRepository extends StorageRepository {
  ...
  getCapabilities(): LaikaTask.LaikaTask<Capabilities> {
    return LaikaTask.succeed<Capabilities>({
      ...
      fileExtensions: {
        supported: true,
        description: 'Each object is one LDP RDF resource at <podRoot>/<key>.<ext>; the extension is preserved as the URL suffix.',
        supportedExtensions: this.serializerRegistry,
      },
      pagination: {
        supported: true,
        description: 'In-memory slicing over LDP container Turtle listings; native pagination via Link headers (RFC 5988) not yet pushed down.',
