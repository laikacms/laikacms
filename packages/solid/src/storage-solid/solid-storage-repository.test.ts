...
const installRoot = () => { containers.add(POD_ROOT); };

const nextEtag = (() => {
  let n = 0;
  return () => { n += 1; return `"${n.toString(16).padStart(8, '0')}"`; };
})();
...
describe('Turtle parser', () => {
  ...
  it('round-trips through serializeTurtle + parseTurtle', () => {
    const triples: TurtleTriple[] = [
      ...
      { subject: 'https://x.test/a/foo', predicate: 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', object: 'http://www.w3.org/ns/ldp#Resource' },
    ];
