...
export class BitbucketStorageRepository extends StorageRepository {
  ...
  > {
    return Effect.gen({ self: this }, function*() {
      ...
      const filtered = listing.success.filter(entry =>
        this.excludeFilter.every(re => !re.test(entry.path))
      );
  ...
  getCapabilities(): LaikaTask.LaikaTask<Capabilities> {
    return LaikaTask.succeed<Capabilities>({
      ...
      fileExtensions: {
        supported: true,
        description: 'Stores each object as a file in the Bitbucket repository using any registered serializer extension.',
