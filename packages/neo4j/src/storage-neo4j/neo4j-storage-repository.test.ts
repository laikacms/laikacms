...
const collectAtomNodes = (filterFn: (n: MockNode) => boolean): MockNode[] =>
  [...nodes.values()].filter(filterFn);

const hasOutgoingChildOf = (path: string): boolean =>
  edges.some(e => e.from === path && e.type === 'CHILD_OF');
...
const evalStatement = (statement: CypherStatement): CypherResult => {
  ...
  // ---- CREATE (f:LaikaFile {…}) RETURN f -------------------------------
  let m = s.match(/^CREATE \(f:LaikaFile \{path: \$path, parent: \$parent, name: \$name, extension: \$extension, content: \$content, createdAt: \$now, updatedAt: \$now\}\) RETURN f$/);
  if (m) {
  ...
  const linkMatch = s.match(/^MATCH \(f:(LaikaFile|LaikaFolder) \{path: \$path\}\) MERGE \(p:LaikaFolder \{path: \$parent\}\)/);
  ...
  if (/^MERGE \(f:LaikaFolder \{path: \$path\}\) ON CREATE SET f\.name = \$name, f\.parent = \$parent, f\.createdAt = \$now, f\.updatedAt = \$now$/.test(s)) {
  ...
  if (/^MATCH \(([a-z])\) WHERE \(\1:LaikaFile OR \1:LaikaFolder\) AND NOT \(\1\)-\[:CHILD_OF\]->\(\) RETURN \1( LIMIT 1)?$/.test(s)) {
...
describe('Neo4jStorageRepository', () => {
  ...
  it('label validation rejects unsafe Cypher labels (no injection)', async () => {
    ...
    expect(() => new Neo4jStorageRepository({
      dataSource: ds,
