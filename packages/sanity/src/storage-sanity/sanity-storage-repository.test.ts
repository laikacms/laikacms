...
const createMockSanity = () => {
  ...
  const matchDocs = (predicate: (doc: StoredDoc) => boolean): StoredDoc[] =>
    [...docs.values()].filter(predicate);
  ...
  const runQuery = (query: string, params: Record<string, unknown>): StoredDoc[] => {
    ...
    if (q === `*[_type == $type && parent == $parent && name in $names][0..1]`) {
      ...
      return matchDocs(d =>
        d._type === params.type && d.parent === params.parent && names.includes(String(d.name)),
      ).slice(0, 2);
    }
    if (q === `*[_type == $type && parent == $parent && name == $name][0..0]`) {
      return matchDocs(d =>
        d._type === params.type && d.parent === params.parent && d.name === params.name,
      ).slice(0, 1);
    }
    if (q === `*[(_type == $folder || _type == $file) && parent == $parent && (name == $name || name match $namePattern)][0..0]`) {
...
beforeEach(() => { mock = createMockSanity(); });
afterEach(() => { mock.docs.clear(); });
