...
const createMockFirestore = () => {
  ...
  const fullName = (path: string) =>
    `projects/${PROJECT}/databases/${DB}/documents/${path}`;
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    if (method === 'GET' && isCollectionPath(path)) {
      ...
      for (const [docPath, doc] of docs) {
        ...
        documents.push({ name: fullName(docPath), fields: doc.fields, createTime: doc.createTime, updateTime: doc.updateTime });
      }
    ...
    if (method === 'PATCH') {
      ...
      return json({ name: fullName(path), fields: body.fields ?? {}, createTime: existing?.createTime ?? now, updateTime: now });
    }
    if (method === 'DELETE') {
      docs.delete(path);
      return json({});
...
beforeEach(() => { mock = createMockFirestore(); });
