...
const parseFilter = (input: string): Filter => {
  ...
  const skipWs = () => { while (pos < input.length && input[pos] === ' ') pos += 1; };
...
const evalFilter = (entry: StoredEntry, filter: Filter): boolean => {
  switch (filter.kind) {
    case 'and': return filter.filters.every(f => evalFilter(entry, f));
    case 'or':  return filter.filters.some(f => evalFilter(entry, f));
    case 'not': return !evalFilter(entry, filter.filter);
    case 'present':
