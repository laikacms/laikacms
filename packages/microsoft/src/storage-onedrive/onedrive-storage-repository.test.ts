...
const nextId = (): string => { idCounter += 1; return `item${idCounter}`; };
...
const handleRequest = (
  ...
  if (parsed.kind === 'metadata' && method === 'GET') {
    if (parsed.path === '') {
      return { status: 200, body: itemToGraphResponse({
        id: 'root',
        name: 'root',
        path: '',
        type: 'folder',
...
const mockFetch: typeof fetch = async (input, init) => {
  ...
  if (url.startsWith(`${CDN}/dl/`)) {
    ...
    return new Response(item.content ?? '', { status: 200, headers: { 'content-type': item.mimeType ?? 'application/octet-stream' } });
  }
  ...
  if (url === `${API}/$batch` && method === 'POST') {
    batchCallCount += 1;
    const body = JSON.parse(init?.body as string) as { requests: Array<{ id: string; method: string; url: string; body?: unknown }> };
  ...
  if (init?.body) {
    ...
    if (init.body && typeof init.body === 'string' && init.body.startsWith('{')) {
      try { parsedBody = JSON.parse(init.body); } catch { /* keep as string */ }
    }
  }
