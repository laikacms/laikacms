...
import { ArangoDataSource, type ArangoCursorResponse, type ArangoDocMeta } from './arango-datasource.js';
...
const evalAql = (query: string, bindVars: Record<string, unknown>): unknown[] => {
  ...
  let m = q.match(/^FOR doc IN (\w+) FILTER doc\.type == @type AND doc\.parent == @parent AND doc\.name == @name LIMIT 1 RETURN doc$/);
  if (m) {
    const coll = collections.get(m[1]!);
    if (!coll) return [];
    const found = [...coll.values()].find(
      d => d.type === bindVars['type']
...
const mockFetch: typeof fetch = async (input, init) => {
  ...
  if (m && method === 'POST') {
    ...
    if (existing && overwrite === 'conflict') {
      return new Response(JSON.stringify({
        errorMessage: 'unique constraint violated', errorNum: 1210, error: true,
      }), { status: 409 });
    }
...
describe('ArangoStorageRepository', () => {
  ...
  it('collection-name validation rejects injection patterns', async () => {
    ...
    expect(() => new ArangoStorageRepository({
      dataSource: ds,
