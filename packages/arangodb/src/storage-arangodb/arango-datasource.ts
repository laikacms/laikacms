...
const errorForResponse = <T>(status: number, body: string, context: string): LaikaResult<T> => {
  ...
  switch (status) {
    ...
    case 409:
      return Result.fail(new EntryAlreadyExistsError(`ArangoDB conflict (errorNum=${errorNum}) for ${context}${detail}`));
    case 429:
...
export class ArangoDataSource {
  ...
  async upsertDocument<T extends { _key: string }>(
    ...
    const url = `${this.apiUrl}/_db/${encodeURIComponent(this.database)}/_api/document/${encodeURIComponent(collection)}?overwriteMode=${overwrite}&returnNew=true`;
    let response: Response;
    try {
  ...
  async getDocument<T = Record<string, unknown>>(
    ...
    const url = `${this.apiUrl}/_db/${encodeURIComponent(this.database)}/_api/document/${encodeURIComponent(collection)}/${encodeURIComponent(key)}`;
    let response: Response;
    try {
