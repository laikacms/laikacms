...
export class PostgrestStorageRepository extends StorageRepository {
        ...
        if (existing) {
          return yield* Effect.fail(
            new EntryAlreadyExistsError(
              `An object with key "${create.key}" already exists with extension .${(existing as Record<string, unknown>)['Extension']}`,
            ),
          );
        }
  ...
  getCapabilities(): LaikaTask.LaikaTask<Capabilities> {
    return LaikaTask.succeed<Capabilities>({
      ...
      pagination: {
        supported: true,
        description: 'In-memory slicing over PostgREST `?Parent=eq.<folder>` queries; cursor pagination is not exposed.',
