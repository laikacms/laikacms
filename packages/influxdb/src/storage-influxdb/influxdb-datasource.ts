...
export class InfluxDbDataSource {
  ...
  async write(points: ReadonlyArray<InfluxPoint>): Promise<LaikaResult<void>> {
    ...
    const body = points
      .map(p => serializeLineProtocolPoint({
      ...
      .join('\n');
    const url = `${this.apiUrl}/api/v2/write?org=${encodeURIComponent(this.org)}&bucket=${encodeURIComponent(this.bucket)}&precision=ns`;
  ...
  async delete(predicate: InfluxDeletePredicate): Promise<LaikaResult<void>> {
    const url = `${this.apiUrl}/api/v2/delete?org=${encodeURIComponent(this.org)}&bucket=${encodeURIComponent(this.bucket)}`;
