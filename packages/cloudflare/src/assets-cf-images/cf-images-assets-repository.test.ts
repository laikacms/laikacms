...
beforeEach(() => { mock = createMockCfImages(); });
afterEach(() => { mock.images.clear(); });
  ...
  it('honours `deliveryUrl` override (custom Worker-fronted domain)', async () => {
    const repo = new CloudflareImagesAssetsRepository({
      ...
      deliveryUrl: ({ imageId, variant }) =>
        `https://cdn.example.com/${variant}/${imageId}.webp`,
...
describe('CloudflareImagesAssetsRepository virtual folders', () => {
  it('synthesises folders from id prefixes', async () => {
    ...
    await LaikaTask.runPromise(repo.createAsset({ key: 'photos/sub/nested', content: tinyPng(), mimeType: 'image/png' }));
