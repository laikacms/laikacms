...
export class GoogleDriveDataSource {
  ...
  async getFileMeta(fileId: string): Promise<LaikaResult<DriveFile>> {
    try {
      const response = await this.request(
        'GET',
        `${DRIVE_API_URL}/files/${encodeURIComponent(fileId)}?fields=${encodeURIComponent(FILE_FIELDS)}&supportsAllDrives=true`,
      );
  ...
  async findChild(parentId: string, name: string): Promise<LaikaResult<DriveFile | null>> {
    ...
    try {
      const response = await this.request(
        'GET',
        `${DRIVE_API_URL}/files?q=${encodeURIComponent(q)}&fields=${encodeURIComponent(LIST_FIELDS)}&pageSize=2&supportsAllDrives=true&includeItemsFromAllDrives=true`,
      );
  ...
  async createFile(
    ...
    try {
      const response = await this.request(
        'POST',
        `${DRIVE_UPLOAD_URL}/files?uploadType=multipart&fields=${encodeURIComponent(FILE_FIELDS)}&supportsAllDrives=true`,
        {
          body,
  ...
  async updateFileContent(
    ...
    try {
      const response = await this.request(
        'PATCH',
        `${DRIVE_UPLOAD_URL}/files/${encodeURIComponent(fileId)}?uploadType=media&fields=${encodeURIComponent(FILE_FIELDS)}&supportsAllDrives=true`,
        {
          body: content,
