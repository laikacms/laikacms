...
export class B2StorageRepository extends StorageRepository {
    ...
    return LaikaTask.make<StorageObject>(() =>
      Effect.gen({ self: this }, function*() {
        ...
        yield* liftResult(this.dataSource.uploadFile(
          fileName, serialized, contentTypeFor(extension),
        ));
    ...
    return LaikaTask.make<StorageObject>(() =>
      Effect.gen({ self: this }, function*() {
        ...
        if (update.content) {
          ...
          yield* liftResult(this.dataSource.uploadFile(
            existing.file.fileName, serialized, contentTypeFor(existing.extension),
          ));
        }
        ...
        const results = yield* Effect.promise(async () => {
          return await Promise.all(cleanKeys.map(async (k) => {
            ...
            const del = await this.dataSource.deleteFileVersion(
              resolved.file.fileName, resolved.file.fileId,
            );
  ...
  getCapabilities(): LaikaTask.LaikaTask<Capabilities> {
    return LaikaTask.succeed<Capabilities>({
      ...
      fileExtensions: {
        supported: true,
        description: 'Each object is one B2 file; the extension is preserved in the file name. File versions accumulate per fileName.',
        supportedExtensions: this.serializerRegistry,
      },
      pagination: {
        supported: true,
        description: 'In-memory slicing over B2 list responses; native `nextFileName` cursor pagination not yet pushed down.',
