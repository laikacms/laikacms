...
const createMockServer = () => {
  ...
  const directChildren = (parent: string): Array<{ name: string; path: string; type: 'tree' | 'blob' }> => {
    ...
    for (const path of files.keys()) {
      ...
      for (let i = 0; i < parentSegs.length; i++) {
        if (segs[i] !== parentSegs[i]) { mismatch = true; break; }
      }
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    const fileMatch = pathname.match(new RegExp(`^${projectPrefix.replace(/\//g, '\\/')}\\/repository\\/files\\/(.+)$`));
