...
function makeConfig(
  ...
  return {
    // The `ai` module is mocked, so the model is never called; a stub stands in
    // for the provider-branded LanguageModel type.
    model: {} as unknown as DecapAiConfig['model'],
...
describe('decapAi()', () => {
  ...
  describe('POST /ai/chat', () => {
    ...
    it('consumer config.tools are merged alongside built-in documentTools, not replacing them', async () => {
      ...
      const customTool = { description: 'custom' } as unknown as NonNullable<DecapAiConfig['tools']>[string];
      const adapter = decapAi(makeConfig({ tools: { myCustomTool: customTool } }));
