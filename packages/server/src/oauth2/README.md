# @laikacms/server/oauth2

[![npm](https://img.shields.io/npm/v/@laikacms/server)](https://www.npmjs.com/package/@laikacms/server)
[![npm](https://img.shields.io/npm/dm/@laikacms/server)](https://www.npmjs.com/package/@laikacms/server)
...
const oauth2 = laikaOauth2({
  ...
  // Required: only these URIs may receive an authorization code (RFC 6749 §3.1.2.2).
  // Omitting this field is a TypeScript error — it is intentionally required.
...
`StoredCredential` and `StoredChallenge` are exported from `@laikacms/server/oauth2`.
...
const oauth2 = laikaOauth2({
  ...
  totp: {
    ...
    required: false,
    /**
     * Issuer name shown in authenticator apps. Defaults to `DEFAULT_TOTP_ISSUER`
     * ('Laika CMS'). BREAKING in v4: this default was 'Decap CMS'. It is baked
     * into every already-minted `otpauth://` URI, so an existing deployment that
     * never set this must now pass `issuer: 'Decap CMS'` to keep enrolled users
     * matching.
     */
    issuer: 'My CMS',
...
`EmailProvider` and `PasswordResetToken` are exported from `@laikacms/server/oauth2`.
...
(`en`) and Dutch (`nl`) out of the box; import them from the `@laikacms/server/oauth2/i18n` subpath.

```typescript
import { laikaOauth2 } from '@laikacms/server/oauth2';
import { nl } from '@laikacms/server/oauth2/i18n';
...
```typescript
import type { OAuthMessages } from '@laikacms/server/oauth2';
...
Both `OAuthMessages` and `Translation` are exported from `@laikacms/server/oauth2/i18n`.

## Additional options

These top-level `OAuthConfig` fields are independent of any optional feature.

| Option                   | Type                                  | Default                  | Description                                                                                                                                                                                                                                |
| ------------------------ | ------------------------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `allowedRedirectUris`    | `string[]`                            | **required**             | Redirect URIs that may receive an authorization code, matched by exact string (RFC 6749 §3.1.2.2). Accepting an arbitrary `redirect_uri` enables auth-code theft — this list is the control. Example: `['https://cms.example.com/admin/']` |
...
| `ignoreUnsafeReasons`    | `UnsafeReason[]`                      | —                        | Safety-gate codes to suppress. Each entry acknowledges a known risk; every suppressed reason is warned about through `logger` on startup. See `UNSAFE_REASONS` for the registry.                                                           |
| `threatModel`            | `'standard' \| 'post-quantum'`        | `'standard'`             | `'standard'` defends against a classical attacker. `'post-quantum'` also enables guards against a cryptographically relevant quantum computer (Shor-breakable primitives are gated).                                                       |
...
| `loginRedirectUrl`       | `string`                              | OAuth authorize endpoint | URL users are sent to after a successful password reset (e.g. your CMS admin page). Defaults to `<basePath>/authorize` when omitted.                                                                                                       |
