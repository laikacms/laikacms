...
export class TrelloStorageRepository extends StorageRepository {
    ...
    return Effect.gen({ self: this }, function*() {
      ...
      if (ownList) {
        ...
        for (const card of cards) {
          ...
          for (const ext of this.availableExtensions) {
            if (stripped.endsWith(`.${ext}`)) { stripped = stripped.slice(0, -(ext.length + 1)); break; }
          }
          summaries.push({
            type: 'object-summary',
  ...
  getCapabilities(): LaikaTask.LaikaTask<Capabilities> {
    return LaikaTask.succeed<Capabilities>({
      ...
      fileExtensions: {
        supported: true,
        description: 'Each object is one Trello card; the extension is encoded into the card name and tracked separately.',
