...
const createMockHygraph = () => {
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    const body = JSON.parse((init?.body as string) ?? '{}') as {
      query?: string; variables?: Vars; operationName?: string;
    };
    ...
    switch (op) {
      case 'FindLaikaObject': {
        ...
        const hit = [...files.values()].find(f =>
          f.parent === vars.parent && names.includes(f.name),
        );
      ...
      case 'FindLaikaFolderByParentName': {
        const hit = [...folders.values()].find(f =>
          f.parent === vars.parent && f.name === vars.name,
        );
        return json({ laikaFolders: hit ? [hit] : [] });
      }
...
beforeEach(() => { mock = createMockHygraph(); });
afterEach(() => { mock.files.clear(); mock.folders.clear(); });
