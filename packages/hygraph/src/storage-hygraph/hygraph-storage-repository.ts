    ...
    return LaikaStream.make<string, RemoveAtomsDone>(emit =>
      Effect.gen({ self: this }, function*() {
        ...
        for (const key of keys) {
          ...
          if (folder) {
            const children = yield* Effect.result(liftResult(this.dataSource.graphql<{ laikaObjects: FileNode[]; laikaFolders: FolderNode[] }>(
              LIST_CHILDREN_QUERY,
            ...
            const deleted = yield* Effect.result(liftResult(this.dataSource.graphql(
              DELETE_FOLDER_MUTATION, { id: folder.id }, 'DeleteLaikaFolder',
            )));
          ...
          const deleted = yield* Effect.result(liftResult(this.dataSource.graphql(
            DELETE_FILE_MUTATION, { id: file.success.id }, 'DeleteLaikaObject',
          )));
          if (Result.isFailure(deleted)) {
            yield* emit.recoverableError(deleted.failure);
            skipped += 1;
    ...
    return Effect.gen({ self: this }, function*() {
      ...
      if (trimmed !== '') {
        const folder = yield* liftResult(this.dataSource.graphql<{ laikaFolders: FolderNode[] }>(
          GET_FOLDER_QUERY, { path: trimmed, stage: this.stage }, 'GetLaikaFolder',
        ));
      ...
      const children = yield* liftResult(this.dataSource.graphql<{
        ...
        LIST_CHILDREN_QUERY, { parent: trimmed, stage: this.stage }, 'ListLaikaChildren',
      ));
