...
export class MeiliDataSource {
  ...
  async search(
    ...
    if (Result.isFailure(resp)) return Result.fail(resp.failure);
    if (!resp.success.ok) return errorForResponse(resp.success.status, await safeText(resp.success), `search ${indexUid}`);
