...
const mockFetch: typeof fetch = async (input, init) => {
  ...
  if (path === '/indexes' && method === 'POST') {
    ...
    if (indexes.has(body.uid)) {
      ...
      return new Response(JSON.stringify({
        taskUid, indexUid: body.uid, status: 'enqueued',
        type: 'indexCreation', enqueuedAt: new Date().toISOString(),
      }), { status: 202 });
    }
  ...
  if (m && method === 'PUT') {
    ...
    completedTasks.add(taskUid);
    return new Response(JSON.stringify({
      taskUid, indexUid: uid, status: 'enqueued',
      type: 'documentAdditionOrUpdate', enqueuedAt: new Date().toISOString(),
    }), { status: 202 });
  }
  ...
  if (m && method === 'POST') {
    ...
    return new Response(JSON.stringify({
      taskUid, indexUid: uid, status: 'enqueued',
      type: 'documentDeletion', enqueuedAt: new Date().toISOString(),
    }), { status: 202 });
  }
  ...
  if (m && method === 'PUT') {
    ...
    return new Response(JSON.stringify({
      taskUid, indexUid: uid, status: 'enqueued',
      type: 'settingsUpdate', enqueuedAt: new Date().toISOString(),
    }), { status: 202 });
  }
  ...
  if (m && method === 'GET') {
    ...
    if (completedTasks.has(taskUid)) {
      return new Response(JSON.stringify({
        uid: taskUid, status: 'succeeded', type: 'unknown',
        enqueuedAt: new Date().toISOString(),
      }), { status: 200 });
    }
    if (completedTasks.has(-taskUid)) {
      return new Response(JSON.stringify({
        uid: taskUid, status: 'failed', type: 'unknown',
        enqueuedAt: new Date().toISOString(),
...
describe('MeiliStorageRepository', () => {
  ...
  it('index uid is validated against an injection-safe regex', async () => {
    ...
    expect(() => new MeiliStorageRepository({
      dataSource: ds,
