...
export class TrelloDataSource {
  ...
  async listBoardLists(): Promise<LaikaResult<TrelloList[]>> {
    ...
    if (!response.ok) {
      return errorForResponse(response.status, await safeText(response), `lists in board ${this.boardId}`);
    }
  ...
  async createList(name: string, options: { pos?: number | 'top' | 'bottom' } = {}): Promise<LaikaResult<TrelloList>> {
    ...
    let response: Response;
    try {
      response = await this.request('POST', `${this.apiUrl}/lists`, {
        body: form,
  ...
  async createCard(
    ...
    let response: Response;
    try {
      response = await this.request('POST', `${this.apiUrl}/cards`, {
        body: form,
        contentType: 'application/x-www-form-urlencoded',
      });
    } catch (cause) {
