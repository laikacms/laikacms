  ...
  getCapabilities(): LaikaTask.LaikaTask<Capabilities> {
    return LaikaTask.succeed<Capabilities>({
      ...
      fileExtensions: {
        supported: true,
        description:
          'Each object is one row in the Convex `laika_files` table; the extension is stored in the `extension` field.',
