  ...
  getObject(key: string): LaikaTask.LaikaTask<StorageObject> {
    return LaikaTask.make<StorageObject>(() =>
      Effect.gen({ self: this }, function*() {
        ...
        const content = yield* Effect.promise(() => this.deserialize(found.extension, String(found.doc.content ?? '')));
        return {
          type: 'object',
          key: trimSlashes(key),
  ...
  getCapabilities(): LaikaTask.LaikaTask<Capabilities> {
    return LaikaTask.succeed<Capabilities>({
      ...
      fileExtensions: {
        supported: true,
        description:
          "Stores each object as a Sanity document of `_type: 'laikaObject'` with a serialized `content` field.",
        supportedExtensions: this.serializerRegistry,
      },
      pagination: {
        supported: true,
        description:
          'In-memory slicing over a GROQ `*[…&& parent == $parent]` query; cursor pagination is not exposed.',
