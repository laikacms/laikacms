...
import { pinataContractCase } from './testing/index.js';

runStorageRepositoryContract(pinataContractCase);
...
describe('PinataStorageRepository folder semantics', () => {
  it('refuses to delete a non-empty folder (recoverable warning, not fatal)', async () => {
    ...
    expect([...mock.pins.values()].some(p => p.metadata.keyvalues.type === 'folder' && p.metadata.name === 'notes'))
      .toBe(true);
  });
