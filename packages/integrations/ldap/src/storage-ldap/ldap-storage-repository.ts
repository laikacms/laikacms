  ...
  getCapabilities(): LaikaTask.LaikaTask<Capabilities> {
    return LaikaTask.succeed<Capabilities>({
      ...
      fileExtensions: {
        supported: true,
        description:
          'Each object is one LDAP entry; the extension is encoded into the cn RDN and stored separately in laikaExtension.',
        supportedExtensions: this.serializerRegistry,
      },
      pagination: {
        supported: true,
        description:
          'In-memory slicing over LDAP search results; native server-side paging via the LDAP Paged Results control (RFC 2696) not yet pushed down.',
