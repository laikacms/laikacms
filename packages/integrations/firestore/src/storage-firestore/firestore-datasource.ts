...
export class FirestoreDataSource {
  ...
  private fullResource(path: string): string {
    return `${this.apiUrl}/projects/${encodeURIComponent(this.projectId)}/databases/${
  ...
  async listCollection(segments: readonly string[]): Promise<LaikaResult<FirestoreDocument[]>> {
    ...
    do {
      ...
      if (!response.ok) {
        return errorForResponse(response.status, await safeText(response), segments.join('/') || '<root>');
      }
