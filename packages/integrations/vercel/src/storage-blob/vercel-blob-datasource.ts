...
export class VercelBlobDataSource {
  ...
  async list(
    ...
    if (!response.ok) {
      return errorForResponse(response.status, await safeText(response), `list(${options.prefix ?? ''})`);
    }
 ...
 */
const encodePathname = (pathname: string): string => pathname.split('/').map(encodeURIComponent).join('/');
