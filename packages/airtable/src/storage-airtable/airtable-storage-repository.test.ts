...
class FormulaParser {
  ...
  private parseFn(kind: 'and' | 'or', skip: number): Expr {
    ...
    while (true) {
      ...
      if (this.s[this.i] === ')') { this.i += 1; break; }
...
const createMockAirtable = () => {
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    if (method === 'PATCH') {
      updateCalls += 1;
      const body = JSON.parse((init?.body as string) ?? '{}') as { records: Array<{ id: string; fields: Partial<Fields> }> };
      if (body.records.length > AIRTABLE_BATCH_LIMIT) {
        return new Response('too many', { status: 422 });
      }
...
beforeEach(() => { mock = createMockAirtable(); });
afterEach(() => { mock.records.clear(); });
...
describe('AirtableStorageRepository CRUD', () => {
  ...
  it('auto-creates ancestor folder records for deep keys', async () => {
    ...
    expect([...mock.records.values()].some(r =>
      r.fields.Type === 'file' && r.fields.Path === 'a/b/c',
    )).toBe(true);
  });
});
