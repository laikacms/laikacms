...
export class CatalogDocumentsRepository extends DocumentsRepository {
  constructor(
    private readonly storageRepository: StorageRepository,
    private readonly settingsProvider: CatalogProvider,
  ) {
    super();
  }
    ...
    return Effect.gen({ self: this }, function*() {
      ...
      if (!statusConfig) {
        ...
        const hint = available
          ? ''
          : ' Configure unpublishedStatuses in your CatalogProvider (see DocumentCollectionSettings.unpublishedStatuses).';
      ...
      const basePath = `.laika/${collection}/${statusConfig.directory}`;
  ...
  private getRevisionPath(
    ...
    return Effect.gen({ self: this }, function*() {
      ...
      const revisionDirectory = settings.revisionDirectory || `.laika/${collection}/revisions`;
  ...
  private listRecordsInternal<T extends Record | RecordSummary>(
    ...
    return LaikaStream.make<T, ListRecordsDone>(emit =>
      Effect.gen({ self: this }, function*() {
        ...
        if (!options.folder) {
          const settings = yield* runForwarding(this.settingsProvider.getCatalog(), emit);
          let count = 0;
