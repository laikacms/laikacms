...
function makeSettingsProvider(overrides?: Partial<DocumentCollectionSettings>): CatalogProvider {
 ...
 */
function makeMultiCollectionProvider(): CatalogProvider {
...
describe('CatalogDocumentsRepository', () => {
  let storage: StorageRepository;
  let settings: CatalogProvider;
  let repo: CatalogDocumentsRepository;

  beforeEach(() => {
    storage = makeMemoryStorage();
    settings = makeSettingsProvider();
    repo = new CatalogDocumentsRepository(storage, settings);
  });
  ...
  describe('listRecords', () => {
    ...
    it('empty folder lists collections as folder entries (LCMS-268)', async () => {
      const repoMulti = new CatalogDocumentsRepository(storage, makeMultiCollectionProvider());
  ...
  describe('updateUnpublished', () => {
    ...
    it('applies content + language edits when changing status in the same call', async () => {
      ...
      const multiStatusRepo = new CatalogDocumentsRepository(
        multiStatusStorage,
...
describe('CatalogDocumentsRepository — EntryAlreadyExistsError detail remapping (LCMS-283/284)', () => {
  ...
  it('createDocument remaps EntryAlreadyExistsError detail to domain key, not storage path (LCMS-283)', async () => {
    ...
    const repo = new CatalogDocumentsRepository(storage, customSettings);
  ...
  it('createUnpublished remaps EntryAlreadyExistsError detail to domain key, not .laika/ path (LCMS-283)', async () => {
    const storage = makeAlwaysConflictStorage('.laika/posts/draft/hello');
    const repo = new CatalogDocumentsRepository(storage, makeSettingsProvider());
    ...
    if (Result.isFailure(result)) {
      expect(result.failure instanceof EntryAlreadyExistsError).toBe(true);
      expect(result.failure.message).toBe('Already exists: posts/hello');
      expect(result.failure.message).not.toContain('.catalog');
    }
  });

  it('createRevision remaps EntryAlreadyExistsError detail to key/revision, not storage path (LCMS-284)', async () => {
    const storage = makeAlwaysConflictStorage('.laika/posts/revisions/hello/v1');
    const repo = new CatalogDocumentsRepository(storage, makeSettingsProvider());
    ...
    async () => {
      ...
      const storage: StorageRepository = {
        ...
        getObject(key: string) {
          return key === '.laika/posts/draft/hello'
            ? LaikaTask.succeed(makeStorageObject(key, {}))
            : LaikaTask.fail(new NotFoundError(`Not found: ${key}`));
        },
        createObject() {
          return LaikaTask.fail(new EntryAlreadyExistsError('Already exists: .laika/posts/review/hello'));
        },
      } as unknown as StorageRepository;
      const repo = new CatalogDocumentsRepository(storage, settings);
...
describe('CatalogDocumentsRepository — getCapabilities', () => {
