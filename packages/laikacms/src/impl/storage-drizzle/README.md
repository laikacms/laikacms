...
const repo = new DrizzleStorageRepository({
  ...
  callbacks: {
    insert: ({ values }) =>
      db.insert(storageTable).values(values).returning(),
    update: ({ where, values }) =>
      db.update(storageTable).set(values).where(where).returning(),
    delete: ({ where }) =>
      db.delete(storageTable).where(where).returning(),
...
- **Folders.** The repository emulates folders by storing a `.keep` placeholder row
  (`type: 'keep-file'`) when `createFolder` is called. `getFolder` returns a synthetic folder
  object if any child key exists under the given prefix.
...
- **Duplicate keys.** `createObject` fails with `EntryAlreadyExistsError` if the key already
  exists. Use `createOrUpdateObject` for upsert semantics.
