...
export const UnpublishedStatusConfigSchema = S.toStandardSchemaV1(S.Struct({
  /** Directory where documents with this status are stored (relative to .laika/[collection]/) */
  directory: S.String,
...
export const DocumentCollectionSettingsSchema = S.toStandardSchemaV1(S.Struct({
   ...
   * Documents with these statuses are stored in .laika/[collection]/[status.directory]
   */
...
export const CatalogSchema = S.toStandardSchemaV1(S.Struct({
  collections: S.optional(S.Record(S.String, CollectionSettingsSchema)),
}));

export type Catalog = S.Schema.Type<typeof CatalogSchema>;
