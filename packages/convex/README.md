...
[Convex](https://convex.dev)-backed implementations of Laika CMS
contracts. First (and current) export:
**`@laikacms/convex/storage-convex`** — a `StorageRepository` over the
Convex HTTP RPC endpoint.
...
Convex is a reactive database where queries and mutations are
**server-side TypeScript functions**, not query strings. Clients
invoke these by *name* over HTTP. Five architectural traits set it
apart from every prior backend in the Laika suite:

**1. Named-function RPC as the query primitive.** The wire shape is
...
The function name travels in the body under `path`, not the URL.
**First "platform-as-API" backend in the suite** — no SQL, Mango,
Cypher, EdgeQL, or any other query DSL. The query "language" is
TypeScript on the Convex side.

**2. `{status, value | errorMessage}` envelope.** Every response —
regardless of HTTP status — wraps the payload:
...
**First backend with explicit success/error discriminator at the
envelope level** (not just HTTP status). The data source unwraps
this automatically and maps `error` responses to typed Laika errors
...
The repository uses only `query` and `mutation`. **First backend with
read/write/side-effect endpoint distinction.**

**4. Transactional mutations.** Every mutation call runs as one
transaction. `removeAtoms(N)` ships as ONE mutation call
(`laika:removeFiles`) with the full path array; the user's function
deletes N rows inside one transaction. The atomicity lives in the
user-written function, not the wire protocol.

**5. Per-deployment URL.** Each Convex deployment has its own
hostname like `https://<deployment-slug>.convex.cloud`. **No database
name in the URL** — the deployment IS the database.
...
import {
  ConvexDataSource,
  ConvexStorageRepository,
} from '@laikacms/convex/storage-convex';
...
The repository invokes a fixed set of named functions in the user's
Convex project. Copy this reference module into `convex/laika.ts`:
...
const fileShape = {
  path: v.string(), parent: v.string(), name: v.string(),
  extension: v.string(), content: v.string(),
...
export const listChildren = query({
  ...
  handler: async (ctx, { parent }) => {
    ...
    const folders = await ctx.db.query('laika_folders')
      .withIndex('by_parent', q => q.eq('parent', parent)).collect();
    return [
      ...files.map(f => ({ _id: f._id, type: 'file', path: f.path, parent: f.parent, name: f.name, extension: f.extension })),
      ...folders.map(f => ({ _id: f._id, type: 'folder', path: f.path, parent: f.parent, name: f.name })),
    ];
  },
});
...
If the reference module doesn't fit your project layout, override the
function paths:

```ts
new ConvexStorageRepository({
  dataSource, serializerRegistry, defaultFileExtension: 'md',
  functions: {
...
`removeAtoms(N)` ships as ONE mutation call — but the *atomicity*
lives in the user-written Convex function, not the wire protocol.
The wire protocol is just RPC. **Not a new atomic-multi-write
mechanism** in the spirit of SurrealDB's `BEGIN/COMMIT`, Neo4j's
`{statements: [...]}`, or Gel's `FOR ... UNION (...)`. The novelty
here is in the named-function RPC and the platform-as-API model.

## Caveats

- **Functions must exist on the Convex side.** The package can't
  fully manage them — you're responsible for deploying the reference
  module (or your own variant). The function-path option lets you
  rename, but the argument shapes are fixed.
- **No subscriptions.** Convex's headline feature is reactive
  subscriptions over WebSocket. This package uses only HTTP RPC.
- **Convex's `_id` is a short opaque string** (e.g. `j97a8f3...`).
  We surface it as `metadata.revisionId`. It changes when the row is
  deleted-then-recreated — not a content hash like AT Protocol.
- **String-content limit.** Convex limits document size to 1 MiB
  (default). For CMS use cases this is usually fine; for larger
  payloads use Convex File Storage and store a reference here.
