...
export const unbind = (arg: LibSqlArg): unknown => {
  switch (arg.type) {
    ...
    case 'integer': return arg.value;            // keep as string — JS can lose precision past 2^53
...
const errorForResponse = <T>(status: number, body: string, context: string): LaikaResult<T> => {
  ...
  switch (status) {
    case 401: return Result.fail(new AuthenticationError(`libSQL authentication failed for ${context}${detail}`));
    case 403: return Result.fail(new ForbiddenError(`libSQL access denied for ${context}${detail}`));
    case 404: return Result.fail(new NotFoundError(`libSQL not found: ${context}`));
    case 429: return Result.fail(new TooManyRequestsError(`libSQL rate-limited request for ${context}`));
    default:
      if (status >= 500) return Result.fail(new ServiceUnavailableError(`libSQL returned HTTP ${status} for ${context}`));
      return Result.fail(new InternalError(`libSQL returned HTTP ${status} for ${context}${detail}`));
  }
};

const errorForStatement = (err: { message?: string; code?: string }, context: string): InternalError | EntryAlreadyExistsError => {
...
export class LibSqlDataSource {
  ...
  private async pipelineRequest(
    ...
    results: Array<
      | { type: 'ok'; response: { type: 'execute'; result: LibSqlExecuteResult } | { type: 'batch'; result: { step_results: Array<LibSqlExecuteResult | null>; step_errors: Array<{ message?: string } | null> } } }
