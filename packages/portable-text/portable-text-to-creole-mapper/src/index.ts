...
/**
 * WikiCreole 1.0 <-> Portable Text.
 *
 * Creole is the "common wiki creole" spec (http://www.wikicreole.org/) meant
 * to unify wiki dialects. Supported subset:
 *
 *  - Headings:    `= H1 =` ... `====== H6 ======`  (Creole = MediaWiki direction)
 *  - Bold:        `**bold**`
 *  - Italic:      `//italic//`
 *  - Link:        `[[target|label]]` (label optional)
 *  - Bullet:      lines beginning with `*`
 *  - Numbered:    lines beginning with `#`
 *  - Code block:  `{{{ … }}}` on their own lines
 *  - Inline code: `{{{code}}}` on a single line
 *  - Line break:  `\\`        — preserved as `\n` inside a span
 *  - Thematic:    `----`      — out of scope (we drop it)
 */
...
function spanToCreole(span: PortableTextSpan, markDefs: PortableTextMarkDefinition[]): string {
...
function spansToCreole(block: PortableTextBlock): string {
  const markDefs = block.markDefs ?? [];
  return (block.children ?? [])
    .filter((c): c is PortableTextSpan => (c as { _type?: string })._type === 'span')
    .map(span => spanToCreole(span, markDefs))
    .join('');
}
...
function blockToCreole(block: PortableTextBlock | Record<string, unknown>): string {
  const type = (block as { _type?: string })._type;
  if (type === 'block') {
    const tb = block as PortableTextBlock;
    const style = tb.style ?? 'normal';
    const inner = spansToCreole(tb);
...
export function portableTextToCreole(doc: PortableTextDocument): string {
  const parts: string[] = [];
  let i = 0;
  while (i < doc.length) {
    const block = doc[i]!;
    if (isListBlock(block)) {
      ...
      while (i < doc.length && isListBlock(doc[i])) {
        ...
        run.push(`${marker.repeat(level)} ${spansToCreole(b)}`);
        i += 1;
      }
      parts.push(run.join('\n'));
      continue;
    }
    const out = blockToCreole(block);
...
function inlineToSpans(
  ...
  // Convert `\\` line-breaks into real newlines before tokenising marks.
  const prepared = text.replace(/\\\\/g, '\n');

  // Tokenise: link, inline code, bold, italic. Order matters.
  const re = /\[\[([^\]\n|]+)(?:\|([^\]\n]+))?\]\]|\{\{\{([^}\n]+)\}\}\}|\*\*([^*\n]+)\*\*|\/\/([^/\n]+)\/\//g;
  let lastIndex = 0;
  let match: RegExpExecArray | null;
  while ((match = re.exec(prepared)) !== null) {
    if (match.index > lastIndex) emit(prepared.slice(lastIndex, match.index), []);
  ...
  if (lastIndex < prepared.length) emit(prepared.slice(lastIndex), []);
  flush();
  return spans;
}
...
export function creoleToPortableText(input: string): PortableTextDocument {
  const keys = newKeys();
  ...
  while (i < lines.length) {
    ...
    // Multi-line code block delimited by `{{{` / `}}}` each on their own line.
    if (line === '{{{') {
    ...
    // Heading: leading and trailing `=` runs of equal length.
    ...
    // List: `*` (bullet) or `#` (numbered); repetition = nesting level.
...
export const creoleFormat: Format = {
  id: 'creole',
  label: 'WikiCreole',

  toPortableText(value: string): PortableTextDocument {
    if (value === '') return [];
    return creoleToPortableText(value);
  },

  fromPortableText(doc: PortableTextDocument): string {
    return portableTextToCreole(doc);
  },
