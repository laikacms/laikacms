...
export class PocketBaseStorageRepository extends StorageRepository {
  ...
  private async findExistingFile(key: string): Promise<LaikaResult<PocketBaseRecord | null>> {
    ...
    const nameClause = this.availableExtensions
      ...
      .join(' || ');
    const filter = `type = ${escapePbFilterValue(TYPE_FILE)} && parent = ${escapePbFilterValue(parent)} && (${nameClause})`;
    ...
    return LaikaTask.make<StorageObject>(() =>
      Effect.gen({ self: this }, function*() {
        ...
        const content = yield* Effect.promise(() =>
          this.deserialize(extension, String(found.content ?? '')),
        );
        return {
          type: 'object',
          key: trimSlashes(key),
  ...
  getCapabilities(): LaikaTask.LaikaTask<Capabilities> {
    return LaikaTask.succeed<Capabilities>({
      ...
      pagination: {
        supported: true,
        description: 'In-memory slicing over PocketBase\'s `filter=parent="..."` list calls; cursor pagination is not exposed.',
