...
class FilterParser {
  ...
  private consume(token: string): boolean {
    this.skipWs();
    if (this.s.startsWith(token, this.i)) { this.i += token.length; return true; }
    return false;
  }
}
...
describe('PocketBase filter parser (mock)', () => {
  it('parses and evaluates the exact filter shapes the repository emits', () => {
    const record = { id: '1', collectionName: COLLECTION, created: '', updated: '', parent: 'notes', name: 'a.md', type: 'file', path: 'notes/a' };
