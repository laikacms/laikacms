...
/**
 * Polka wraps Node.js IncomingMessage / ServerResponse — the same underlying
 * types as Express. laika.fetch expects a WHATWG Fetch API Request, so we
 * reconstruct one from the raw stream.
 *
 * Polka is used internally by SvelteKit's Vite dev server (via vite-dev-rpc),
 * making this pattern relevant to many SvelteKit developers who embed custom
 * API routes without pulling in a full framework.
 */
...
app.all('/api/decap/*', async (req: IncomingMessage, res: ServerResponse) => {
...
// Blog index.
app.get('/', async (_req: IncomingMessage, res: ServerResponse) => {
...
// Blog post.
app.get('/blog/:slug', async (req: IncomingMessage & { params: Record<string, string> }, res: ServerResponse) => {
...
app.listen(PORT, () => {
  console.log(`Polka blog running at http://localhost:${PORT}`);
