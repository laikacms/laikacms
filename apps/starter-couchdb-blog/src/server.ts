...
const ADMIN_HTML = decapAdminHtml({
  decapConfig,
  title: 'Admin · LaikaCMS CouchDB starter',
});

app.get('/', c =>
  c.json({
    name: '@laikacms/starter-couchdb-blog',
    storage: `CouchDB (${process.env['COUCH_URL'] ?? 'http://localhost:5984/cms'})`,
    endpoints: {
      'GET /': 'this index',
...
serve({ fetch: app.fetch, port: PORT }, info => {
  console.log(`LaikaCMS CouchDB blog running at http://localhost:${info.port}`);
