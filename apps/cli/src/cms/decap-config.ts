/**
 * The Decap adapter's typed-config codegen: turn a Decap `config.yml` into a
 * TypeScript module that is both the parsed value and the literal types
 * inferred from it. Reached through `decapAdapter.config`, never directly.
 */
...
import type { CmsConfigDiscovery } from './types.js';
 ...
 */
export async function discoverConfig(cwd: string): Promise<CmsConfigDiscovery> {
...
export async function loadConfig(inputPath: string): Promise<unknown> {
  ...
  return load(raw, { schema: JSON_SCHEMA });
}
...
export function serialize(obj: unknown, options: SerializeOptions): string {
  const header = [
    '// @generated',
    `// AUTO-GENERATED by laikacli — do not edit by hand.`,
