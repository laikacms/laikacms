...
app.get('/admin/', c => c.html(decapAdminHtml({ title: 'Admin · Notion Blog' })));
...
app.get('/', async c => {
  ...
  const rootPageId = process.env['NOTION_ROOT_PAGE_ID'] ?? '(not set)';
...
<html lang="en">
<head><meta charset="utf-8"><title>My Blog · Notion</title></head>
<body>
  <h1>My Blog</h1>
  <p><small>Notion root page: ${rootPageId}</small></p>
  ${body}
...
serve({ fetch: app.fetch, port: PORT }, info => {
  console.log(`Notion blog running at http://localhost:${info.port}`);
  console.log(`  Blog:  http://localhost:${info.port}/`);
  console.log(`  Admin: http://localhost:${info.port}/admin/`);
  console.log(
    `  Root:  ${process.env['NOTION_ROOT_PAGE_ID'] ?? '(NOTION_ROOT_PAGE_ID not set)'}`,
  );
});
