 ...
 * ─────────────────────────
 * Oak wraps the native Deno Request in its own `oak.Request` type, but it
 * exposes the underlying WHATWG Request via `ctx.request.original`. Pass
 * that directly to `laika.fetch` — no body buffering or Request reconstruction
 * needed. The body stream is intact because Oak lazily reads the body (only
 * when you call `ctx.request.body`).
 *
 *   const res = await laika.fetch(ctx.request.original);
 *
 * Compare to Hono / Elysia which also surface the native Request directly, and
 * to Express / Fastify which need a full IncomingMessage → Request adapter.
 *
...
// ctx.request.original is the native WHATWG Request — pass it directly.
// Oak lazily reads the body (only on ctx.request.body calls) so the stream
// is still intact here. No buffering or reconstruction needed.
async function proxyToLaika(ctx: Context): Promise<void> {
  const res = await laika.fetch(ctx.request.original);
  ctx.response.status = res.status;
  res.headers.forEach((v: string, k: string) => {
    // Oak sets transfer-encoding itself; duplicating it causes "invalid header" errors.
    if (k.toLowerCase() !== 'transfer-encoding') ctx.response.headers.set(k, v);
  });
