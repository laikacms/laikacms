...
const ADMIN_HTML = decapAdminHtml({
  decapConfig,
  title: 'Admin · LaikaCMS Pinata IPFS starter',
});

app.get('/', c =>
  c.json({
    name: '@laikacms/starter-pinata-blog',
    storage: 'Pinata IPFS (content-addressed)',
    note: 'pinList is eventually consistent — reads may lag seconds behind writes.',
    endpoints: {
      'GET /': 'this index',
...
serve({ fetch: app.fetch, port: PORT }, info => {
  console.log(`LaikaCMS Pinata IPFS blog running at http://localhost:${info.port}`);
