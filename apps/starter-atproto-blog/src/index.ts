...
app.get('/admin/', c => c.html(decapAdminHtml({ title: 'Admin · ATProto Blog', decapConfig: laika.decapConfig })));
...
app.get('/', async c => {
  ...
  const did = process.env['ATP_DID'] ?? '(not set)';
...
<html lang="en">
<head><meta charset="utf-8"><title>My Blog · AT Protocol</title></head>
<body>
  <h1>My Blog</h1>
  <p><small>AT Protocol repo: <a href="${pdsUrl}">${did}</a></small></p>
  ${body}
...
serve({ fetch: app.fetch, port: PORT }, info => {
  console.log(`ATProto blog running at http://localhost:${info.port}`);
  console.log(`  Blog:  http://localhost:${info.port}/`);
  console.log(`  Admin: http://localhost:${info.port}/admin/`);
  console.log(`  DID:   ${process.env['ATP_DID'] ?? '(ATP_DID not set)'}`);
  console.log(`  PDS:   ${process.env['ATP_PDS_URL'] ?? 'https://bsky.social'}`);
});
