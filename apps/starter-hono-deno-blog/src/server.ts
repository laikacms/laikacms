/**
 * Hono on Deno 2 — Deno.serve(app.fetch), no adapter.
 *
 * Deno passes a WHATWG Request to every fetch handler, so laika.fetch(c.req.raw)
 * requires zero bridging — same as Cloudflare Workers and Bun.serve().
 *
 * serveStatic from hono/deno reads files via Deno's filesystem APIs.
 *
 * Contrast:
 *   starter-hono-blog      — Node, @hono/node-server (needs IncomingMessage→Request bridge internally)
 *   starter-hono-bun-blog  — Bun, export default { fetch: app.fetch }
 *   starter-hono-deno-blog — Deno, Deno.serve(app.fetch)
 *
 * Required Deno permissions:
 *   --allow-net    HTTP server
 *   --allow-read   Serve static files + read content/
 *   --allow-write  Write content/ (Decap CRUD)
 */
...
// Decap JSON:API — Deno passes WHATWG Requests, so no bridging needed.
...
// Deno.serve with a single fetch handler — simplest form.
