...
app.get('/', async c => {
  ...
  const tableName = process.env['AIRTABLE_TABLE'] ?? 'cms';
...
<html lang="en">
<head><meta charset="utf-8"><title>My Blog · Airtable</title></head>
<body>
  <h1>My Blog</h1>
  <p><small>Airtable base: ${baseId} / table: ${tableName}</small></p>
  ${body}
...
serve({ fetch: app.fetch, port: PORT }, info => {
  console.log(`Airtable blog running at http://localhost:${info.port}`);
  console.log(`  Blog:  http://localhost:${info.port}/`);
  console.log(`  Admin: http://localhost:${info.port}/admin/`);
  console.log(`  Base:  ${process.env['AIRTABLE_BASE_ID'] ?? '(AIRTABLE_BASE_ID not set)'}`);
});
