...
// Fly volumes are empty on first attach. The embedded preset auto-creates the
// content dir but only if its PARENT exists — so make sure /data is there.
try {
...
const laika = createEmbeddedLaika({
  ...
  decapConfig,
  basePath: '/api/decap',
  auth: { mode: 'dev' }, // replace before going to prod — see README
});

const ADMIN_HTML = decapAdminHtml({ decapConfig, title: 'Admin · LaikaCMS Fly.io starter' });

const app = new Hono();

app.get('/', c =>
  c.json({
    name: '@laikacms/starter-fly-io',
    runtime: `Node.js ${process.version}`,
    storage: `FileSystem @ ${CONTENT_DIR} (Fly volume in production)`,
...
serve({ fetch: app.fetch, port: PORT, hostname: '0.0.0.0' }, info => {
  // eslint-disable-next-line no-console
  console.log(`LaikaCMS Fly.io backend listening on http://0.0.0.0:${info.port}`);
  // eslint-disable-next-line no-console
