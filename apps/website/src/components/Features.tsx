...
const PILLARS: Pillar[] = [
  ...
  },
  {
    ...
    t: 'Headless by design',
    d: 'Your content lives behind an API, not inside a theme. Render it with React, Vue, Astro, a static site generator, a mobile app, or all of them at once, from the same source of truth.',
  },
  {
    icon: <IconSwap size={20} />,
    t: 'Backend-agnostic: 40+ stores',
  ...
  },
  {
    ...
    t: 'A modular dashboard builder',
    d: (
      <>
        The admin is not a fixed UI: compose your dashboard from ready-made modules and extend it with your own screens.
        It runs production back offices today, like the wholesale portal behind{' '}
        <a
          href="https://superstar.nl"
          target="_blank"
          rel="noreferrer"
          className="text-indigo underline underline-offset-2 hover:text-indigo-700"
        >
          superstar.nl
        </a>.
      </>
    ...
    <section className="border-t border-hairline py-28 max-[760px]:py-[76px] relative">
      <div className="max-w-[1200px] mx-auto px-10 max-[760px]:px-[22px]">
        ...
        <p className="mt-[22px] text-[clamp(17px,1.5vw,20px)] text-ink-2 max-w-[56ch] leading-[1.55]">
          Laika CMS is open-source, headless, backend-agnostic content management. The defining idea: the storage
          backend should be a swappable detail, not an architecture you marry, so it ships with 40+ of them. And it cuts
          both ways: the admin is a modular dashboard you compose and extend, not a UI you inherit. All of it yours,
