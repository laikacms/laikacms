 ...
 *
 * In Nuxt 3 with the cloudflare-pages Nitro preset, the D1 binding arrives
 * via event.context.cloudflare.env.DB — it is not available at module load
 * time, so we cannot use a singleton. Call this per-request.
 */
