...
if (!DATABASE_URL) throw new Error('DATABASE_URL is required (postgresql://user:pass@host:port/db)');

// DrizzleStorageRepository requires async setup (CREATE TABLE on boot).
// createCustomLaika takes any StorageRepository — this is the "BYO storage" pattern.
const storage = await createDrizzleStorage(DATABASE_URL);
...
<html lang="en">
<head><meta charset="utf-8"><title>My Blog · Postgres</title></head>
<body>
  <h1>My Blog</h1>
  <p><small>Storage: DrizzleStorageRepository / node-postgres (pg)</small></p>
  ${body}
...
serve({ fetch: app.fetch, port: PORT }, info => {
  console.log(`Postgres blog running at http://localhost:${info.port}`);
