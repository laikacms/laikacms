...
A blog starter built with [HonoX](https://github.com/honojs/honox) (Hono's file-based
meta-framework) and [LaikaCMS](https://github.com/laikacms/laikacms).

HonoX routes are Web-API-native: `c.req.raw` is a standard `Request`, so `laika.fetch(c.req.raw)`
requires no adapter bridge.
