...
New package: `@laikacms/vercel`. First export
`@laikacms/vercel/storage-blob` — a `StorageRepository` backed by
...
(2) Vercel's list endpoint has no `delimiter` param, so subfolder
grouping is reconstructed client-side. `addRandomSuffix=0` is
