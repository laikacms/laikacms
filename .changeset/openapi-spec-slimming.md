...
members, catalog omitted `code` and `source` entirely, and every spec advertised a `links.last` no
link builder can produce. That vocabulary now lives once in `laikacms/json-api` as
...
`buildCatalogOpenApi` remain exported and unchanged in signature.
...
now `JsonApiErrorObject`, and catalog's `JsonApiError`/`JsonApiErrorDocument` are now
