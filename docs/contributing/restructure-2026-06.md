...
The `apps/` directory was removed during the restructure. **Update (v2.1.0, commit `be41003`):**
`apps/website` (`@laikacms/website` — the laikacms.com marketing site) was subsequently re-added as
a private workspace package; `apps/` now contains only that one entry.
...
- **5 non-starter apps**: `decap-cms-laika-app`, `laika-gateway`, `storage-contract-e2e`,
  `storybook`, and `website`. (`website` was subsequently re-added as `apps/website` in v2.1.0 — see
  the note above.)
