...
| `laikacms/core/errors`       | Domain error classes (`LaikaError` subclasses). Commonly caught: `NotFoundError`, `BadRequestError`, `ForbiddenError`, `InternalError`, `LockConflictError`, `VersionMismatchError`, `EntryAlreadyExistsError`, `UpstreamUnAuthorizedError` (401 from upstream — was `AuthorizationError` before the upstream-unauthorized-error rename; update catch clauses), `AuthenticationError`, `TooManyRequestsError`. Full list (30 classes) in `packages/laikacms/src/shared/core/domain/entities/errors.ts`.                                                                                               |
...
| `laikacms/file-sanitizer`    | Strips privacy-sensitive metadata from image uploads (PNG, GIF, WebP, JPEG). Key exports: `FileSanitizerImpl` (implements `Sanitizer`; wire into assets repositories via DI), `sanitizeFile(data, options?, mimeType?)` (throws `UnsupportedFileTypeError` / `CorruptedFileError` / `FileTooLargeError`), `canSanitize(data)` (non-throwing preflight), `getSupportedFileTypes()` → `['png', 'gif', 'webp', 'jpeg']`. Individual sanitizers: `PngSanitizer`, `GifSanitizer`, `WebpSanitizer`, `JpegSanitizer`. Scanner utilities: `DangerousContentScanner`, `ScanResult`, `scanForDangerousContent`. |
...
| `laikacms/sanitizer`         | Abstract `Sanitizer` interface and companion types — the DI seam that assets repositories (`assets-r2`, etc.) depend on. Exports: `Sanitizer` interface (`sanitize(data, options?, mimeType?)`, `getSupportedFileTypes()`, `canSanitize(data)`), `SanitizeOptions` (`maxFileSize`, `ignoreExtensions`), `SanitizeResult` (`data`, `fileType`, `strippedMetadata`, `ignored`), `StrippedMetadataInfo`. Implement this interface to supply a custom sanitizer; use `laikacms/file-sanitizer` for the built-in image sanitizer.                                                                          |
