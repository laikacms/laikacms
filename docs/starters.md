...
| [`apps/starter-effect-platform-blog`](../apps/starter-effect-platform-blog/) | `@effect/platform-node` HttpRouter — typed Effect routes, WHATWG bridge via `HttpServerRequest.toWeb` / `HttpServerResponse.fromWeb` | FileSystem                        | Decap via `decapAdminHtml()`                              | ✅ Working                                                       |
...
| [`apps/starter-pagination-blog`](../apps/starter-pagination-blog/) | Hono blog demonstrating
**page-based pagination** — uses `collectStream`'s `done.total` and `done.pagination` to render
prev/next links without loading all posts | FileSystem | Decap via `decapAdminHtml()` | ✅ Working |
