...
| [`apps/starter-effect-platform-blog`](../apps/starter-effect-platform-blog/) | `@effect/platform-node` HttpRouter + WHATWG bridge (`HttpServerRequest.toWeb` / `HttpServerResponse.fromWeb`) — demonstrates Effect 4.x HTTP API | FileSystem | Decap via `decapAdminHtml()` | ✅ Working |
| [`apps/starter-workers-d1`](../apps/starter-workers-d1/)               | Cloudflare Workers + Hono + **D1** (SQLite) via `DrizzleStorageRepository` + `createCustomLaika`                         | **D1 (SQLite)**                   | Decap via `decapAdminHtml()`                              | ✅ Working                                                       |
| [`apps/starter-angular-ssr-blog`](../apps/starter-angular-ssr-blog/)   | Angular 21 SSR — Vite + `@analogjs/vite-plugin-angular` + `renderApplication()` + Express (what Analog/Nitro abstracts) | FileSystem                        | Decap via `decapAdminHtml()`                              | ✅ Working                                                       |
