...
| [`apps/starter-portfolio`](../apps/starter-portfolio/)                 | Multi-collection Decap config (Projects + Blog + About singleton) via `minimalBlogConfig({ extraCollections })` — custom field widgets, `tags` list, `files` singleton pattern | FileSystem | Decap via `decapAdminHtml()` | ✅ Working |
