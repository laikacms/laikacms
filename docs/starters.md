...
| [`apps/starter-contentful-blog`](../apps/starter-contentful-blog/)     | Hono + `ContentfulStorageRepository` — structured fields (no serializer), native OCC via `sys.version`, `createFolder` auto-activates a content type | **Contentful**                    | Decap via `decapAdminHtml()`                              | ✅ Working                                                       |
