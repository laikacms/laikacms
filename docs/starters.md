...
| [`apps/starter-custom-auth`](../apps/starter-custom-auth/)             | `mode: 'custom'` auth — API-key bearer tokens validated server-side (Hono). Shows `authenticateAccessToken` + `authenticateApiToken` + `devToken` option on `decapAdminHtml()`. | FileSystem                        | Decap via `decapAdminHtml({ devToken: apiKey })`            | ✅ Working                                                       |
| [`apps/starter-session-auth`](../apps/starter-session-auth/)           | Login page + `httpOnly` session cookie + per-request JWT injection via `decapAdminHtml({ devToken: jwt })`. Uses `jose.jwtVerify` in `authenticateAccessToken`.                 | FileSystem                        | Decap via `decapAdminHtml({ devToken: jwt })` (per-request) | ✅ Working                                                       |
