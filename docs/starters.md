...
AWS Lambda, and many more). They favored the **embedded preset** (`@laikacms/decap/embedded`) and
FileSystem storage so anyone could run them without a cloud account.
...
- `@laikacms/decap/embedded` — `createEmbeddedLaika`, `minimalBlogConfig`, `decapAdminHtml()`
  (Node.js / FileSystem).
- `@laikacms/decap/workers` — `createWorkersLaika` (Cloudflare Workers / R2).
- `@laikacms/decap/custom` — `createCustomLaika({ storage, … })` for any pre-built
  `StorageRepository`.
