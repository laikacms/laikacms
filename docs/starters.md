...
| [`apps/starter-bitbucket-blog`](../apps/starter-bitbucket-blog/)       | Hono + `BitbucketStorageRepository` — all writes go through one `POST /src` multipart endpoint (one atomic commit per op) | **Bitbucket**                     | Decap via `decapAdminHtml()`                              | ✅ Working                                                       |
