...
| [`apps/starter-solid-blog`](../apps/starter-solid-blog/)               | Hono + `SolidStorageRepository` — URI-as-identity, trailing-slash container/resource, RDF/Turtle listings, `If-None-Match:*` OCC | **Solid Pod (LDP)**               | Decap via `decapAdminHtml()`                              | ✅ Working (CommunitySolidServer for local dev)                  |
