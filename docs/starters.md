...
| [`apps/starter-sanity-blog`](../apps/starter-sanity-blog/)             | Hono + `SanityStorageRepository` — GROQ reads, atomic `/mutate` batches, OCC via `_rev` + `ifRevisionID`                 | **Sanity Content Lake**           | Decap via `decapAdminHtml()`                              | ✅ Working                                                       |
