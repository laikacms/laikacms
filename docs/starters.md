...
| [`apps/starter-fresh-blog`](../apps/starter-fresh-blog/)               | **Deno Fresh 1.7** (file-based routing, Preact SSR, islands) — full Fresh vs raw Deno.serve                              | FileSystem                        | Decap via `decapAdminHtml()` (raw Response, no hydration) | 🟡 Scaffolded (Deno not on dev box)                              |
