...
| [`apps/starter-gel-blog`](../apps/starter-gel-blog/)                   | **Gel** (formerly EdgeDB) — EdgeQL typed params (`:=` assign vs `=` compare), `FOR … UNION` atomic batch, schema-first   | **Gel**                           | Decap via `decapAdminHtml()`                              | 🟡 Requires Gel instance                                         |
