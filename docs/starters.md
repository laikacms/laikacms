...
| [`apps/starter-oak-blog`](../apps/starter-oak-blog/)                   | **Oak** (Deno's most popular HTTP framework, JSR) — Router + middleware + `ctx` bridge                                   | FileSystem                        | Decap via `decapAdminHtml()`                              | 🟡 Scaffolded (Deno not on dev box)                              |
| [`apps/starter-angular-blog`](../apps/starter-angular-blog/)           | **Angular 19** SSR (`@angular/ssr` + CommonEngine) — standalone components, signals, HTTP interceptor for SSR            | FileSystem                        | Decap via `decapAdminHtml()` (Express pre-route)          | ✅ Working                                                       |
