...
| [`apps/starter-trello-blog`](../apps/starter-trello-blog/)             | Hono + `TrelloStorageRepository` — cards hold content, lists are folders; query-string auth; float `pos` ordering; soft-delete lists | **Trello**                        | Decap via `decapAdminHtml()`                              | ✅ Working (card desc capped at 16KB)                            |
