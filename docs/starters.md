...
| [`apps/starter-preview-mode`](../apps/starter-preview-mode/)           | Draft preview via `getUnpublished(key)` + token-gated `/drafts` list using `type: 'unpublished'`. Shows the full draft→publish workflow. | FileSystem                        | Decap via `decapAdminHtml()`                              | ✅ Working                                                       |
