...
| [`apps/starter-custom-auth`](../apps/starter-custom-auth/)             | **Production auth** — `mode: 'custom'` with API-key bearer tokens; `decapAdminHtml({ devToken })` for token injection    | FileSystem                        | Decap via `decapAdminHtml({ devToken })`                  | ✅ Working                                                       |
