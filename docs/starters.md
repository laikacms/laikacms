...
| [`apps/starter-microsoft-blog`](../apps/starter-microsoft-blog/)       | Hono + `OneDriveStorageRepository` (Graph API — native path addressing, `$batch` bulk-delete, pre-signed `downloadUrl`)  | **OneDrive / SharePoint**         | Decap via `decapAdminHtml()`                              | ✅ Working                                                       |
