...
| [`apps/starter-nestjs-blog`](../apps/starter-nestjs-blog/)             | NestJS 11 + **Express** adapter (`@nestjs/platform-express`) — DI modules, controllers, esbuild admin bundle                | FileSystem                        | Decap, embedded (esbuild bundle)                          | ✅ Working                                                       |
| [`apps/starter-nestjs-fastify`](../apps/starter-nestjs-fastify/)       | NestJS 11 + **Fastify** adapter (`@nestjs/platform-fastify`) — JSON re-serialization bridge, `decapAdminHtml()` shell       | FileSystem                        | Decap via `decapAdminHtml()`                              | ✅ Working                                                       |
| [`apps/starter-hono-bun`](../apps/starter-hono-bun/)                   | Hono on **Bun** runtime — WHATWG-native `Bun.serve()`, `Bun.file()` static assets, `decapAdminHtml()`, hot-reload           | FileSystem                        | Decap via `decapAdminHtml()`                              | ✅ Working                                                       |
| [`apps/starter-hexo-blog`](../apps/starter-hexo-blog/)                 | **Hexo 8** SSG — LaikaCMS injected as `server_middleware` via `.mjs` script; admin at `/admin`, content in `source/_posts/` | FileSystem                        | Decap via `decapAdminHtml()`                              | ✅ Working (dev server)                                          |
