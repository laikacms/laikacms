...
| [`apps/starter-cloudinary-blog`](../apps/starter-cloudinary-blog/)     | Cloudinary for media + FileSystem for content — shows `decapApi()` direct call to inject a custom `AssetsRepository`     | FileSystem (docs) + **Cloudinary** (media) | Decap via `decapAdminHtml()`                   | 🟡 Requires Cloudinary account                                   |
