...
| [`apps/starter-hygraph-blog`](../apps/starter-hygraph-blog/)           | Hono + `HygraphStorageRepository` — first true-GraphQL transport; one query returns files + folders; stage-aware (DRAFT/PUBLISHED) | **Hygraph**                       | Decap via `decapAdminHtml()`                              | ✅ Working (requires LaikaObject + LaikaFolder models)           |
