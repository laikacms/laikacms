...
| [`apps/starter-draft-preview`](../apps/starter-draft-preview/)         | Hono blog with **draft preview mode** — unpublished posts hidden from visitors, revealed to editors via `?token=` cookie; uses `getUnpublished()` + `listRecords({ type: 'unpublished' })` | FileSystem                        | Decap via `decapAdminHtml()`                              | ✅ Working                                                       |
